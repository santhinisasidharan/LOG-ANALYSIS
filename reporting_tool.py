...
def three_most_popular_articles():
    '''This function prints the three most popular articles of all time in descending order'''
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("SELECT title,views FROM popular_article_details ORDER BY views DESC limit 3;")
    return c.fetchall()
    db.close()

def most_popular_authors():
    '''This function prints the most popular authors based on summed up views for all articles by that author'''
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
...
def more_than_one_percent_error():
    '''This function prints the dates where more than 1% request lead to errors.
        The log table includes a column status that indicates the HTTP status code,
        that the news site sent to the user's browser.'''
    db = psycopg2.connect(database=DBNAME)
    c = db.cursor()
    c.execute("SELECT date,CAST(percentage_error AS DECIMAL(10,2)) FROM percent_error_table;")
    return c.fetchall()
    db.close()
